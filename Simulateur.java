...
public class Simulateur {
	...
	public void simuler() {
		...
		// La voiture retirée de la file d'entrée mais pas encore stationnée
		// (null = aucune voiture en attente de stationnement)
		Emplacement enAttenteDeStationnement = null;

		while (horloge < etapes) {

			// ÉTAPE 1 : Arrivée éventuelle — ajouter à la file d'entrée
			if (GenerateurAleatoire.evenementSurvenu(this.probabiliteArriveeParSeconde)) {
				Emplacement nouvelle = new Emplacement(
					GenerateurAleatoire.genererVoitureAleatoire(), horloge);
				fileEntrante.enfiler(nouvelle);
			}

			// ÉTAPE 2 : Tentative de stationnement
			// Si aucune voiture n'est en attente, on retire la prochaine de la file
			if (enAttenteDeStationnement == null && !fileEntrante.estVide()) {
				enAttenteDeStationnement = fileEntrante.defiler();
			}

			// On tente de stationner la voiture en attente (si elle existe)
			if (enAttenteDeStationnement != null) {
				if (stationnement.tenterStationnement(
						enAttenteDeStationnement.getVoiture(),
						enAttenteDeStationnement.getHorodatage())) {
					// Succès : afficher l'entrée et libérer l'attente
				...
				// Échec : on garde enAttenteDeStationnement pour réessayer la prochaine seconde
			}

			// ÉTAPE 3 : Départ éventuel des voitures déjà stationnées
			for (int i = 0; i < stationnement.getNombreRangees(); i++) {
				for (int j = 0; j < stationnement.getNombrePlacesParRangee(); j++) {
					...
					if (c != null && c.getVoiture() != null) {
						...
						} else if (GenerateurAleatoire.evenementSurvenu(
								pdfDepart.pdf(dureeStationnement))) {
							// Départ probabiliste
							stationnement.retirer(i, j);
							fileSortante.enfiler(c);
						}
			...
			// ÉTAPE 4 : Sortie d'une voiture de la file sortante
...
//             if (stationnement.tenterStationnement(new_Car.getVoiture(), new_Car.getHorodatage())) {
//                 System.out.println(new_Car.getVoiture() + " est ENTRÉE à l'instant " + horloge 
//                     + ";l'occupation est maintenant de " + stationnement.getOccupationTotale());
//             }
//         }
...
//         if (!fileSortante.estVide()) {
//             Emplacement emp = fileSortante.defiler();
//             System.out.println(emp.getVoiture() + " est SORTIE à l'instant " + horloge 
//                 + ";l'occupation est maintenant de " + stationnement.getOccupationTotale());
//         }

//         horloge++;  // <-- incrémenter À LA FIN
//     }
// }
// 	/**
